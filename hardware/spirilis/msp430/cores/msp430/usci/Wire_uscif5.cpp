...
boolean TwoWire::isr(uint16_t intvect, boolean do_txrx_only)
{
	...
	register volatile uint8_t *_ifg = SFRPTR_BYTE uscib_registers[_uscib_offset].UCBxIFG;

	if (intvect >= USCI_I2C_UCRXIFG) {
		// TX/RX
		if (intvect == USCI_UCTXIFG) {
			// TX code
...
void TwoWire::begin(int ouraddr)
{
	...
	volatile uint8_t *ctl1 = SFRPTR_BYTE uscib_registers[_uscib_offset].UCBxCTL1;
	volatile uint16_t *i2coa0 = SFRPTR_WORD uscib_registers[_uscib_offset].UCBxI2COA;
	volatile uint8_t *ie = SFRPTR_BYTE uscib_registers[_uscib_offset].UCBxIE;
	*ctl1 |= UCSWRST;
	*i2coa0 = ouraddr;
	if (ouraddr & 0x180)
		*ctl0 |= UCA10;
	*ctl0 &= ~UCMST;
	*ctl1 &= ~UCSWRST;
	*ie |= UCTXIE | UCRXIE | UCSTTIE | UCSTPIE | UCALIE | UCNACKIE;
}
